    ...
    public class DriverOrdersController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetMyOrders([FromQuery] OrderStatus? status)
        {
            ...
            var orders = await query
                ...
                .Select(o => new BarbariBahar.API.Core.DTOs.Driver.DriverOrderSummaryDto
                {
                    Id = o.Id,
        ...
        public async Task<IActionResult> GetOrderDetail(long id)
        {
            ...
            var orderDetail = new BarbariBahar.API.Core.DTOs.Driver.DriverOrderDetailDto
            {
                Id = order.Id,
                ...
                Customer = new BarbariBahar.API.Core.DTOs.Driver.CustomerContactDto
                {
        ...
        [HttpPatch("{id}/status")]
        public async Task<IActionResult> UpdateOrderStatus(long id, [FromBody] BarbariBahar.API.Core.DTOs.Driver.UpdateOrderStatusDto updateStatusDto)
        {
            var driverIdStr = User.FindFirst(ClaimTypes.NameIdentifier)?.Value;
            if (!long.TryParse(driverIdStr, out var driverId))
            {
