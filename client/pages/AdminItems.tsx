...
export default function AdminItems() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const [resItems, resProds] = await Promise.all([apiFetch(`/api/admin/items`), apiFetch(`/api/admin/products`)]);
  ...
  const addItem = async () => {
    ...
    setSubmitting(true);
    try {
      const res = await apiFetch(`/api/admin/items`, { method: "POST", body: JSON.stringify({ name, productId, quantity: Number(quantity) || 0, price: Number(price) || 0 }) });
      if (!res.ok) {
  ...
  const saveEdit = async () => {
    if (!editingId) return;
    try {
      const res = await apiFetch(`/api/admin/items/${editingId}`, { method: "PATCH", body: JSON.stringify({ name: editName, productId: editProductId, quantity: Number(editQuantity) || 0, price: Number(editPrice) || 0 }) });
              ...
              <div className="overflow-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {loading ? (
                      <tr>
                        <td colSpan={6} className="py-6 text-center text-foreground/60">در حال بارگذاری...</td>
                      </tr>
                    ) : items.length === 0 ? (
                      <tr>
                        <td colSpan={6} className="py-6 text-center text-foreground/60">آیتمی یافت نشد</td>
                      </tr>
                    ) : (
                      items.map((it) => (
